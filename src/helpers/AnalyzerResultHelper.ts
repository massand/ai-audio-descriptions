...
/**
 * Fetches analyzer results from blob storage and converts them to original summary descriptions
 */
export const fetchAndConvertOriginalAnalyzerResults = async (analyzerResultJsonUrl: string, title: string): Promise<Segment[] | null> => {
    try {
        // Fetch the analyzer results from blob storage
        const response = await axios.get(analyzerResultJsonUrl);
        const analyzerResults: ContentUnderstandingResults = response.data;
        
        // Extract segments from the analyzer results with original summaryDescription
        const segments = extractOriginalSegmentsFromAnalyzerResults(analyzerResults);
        
        if (segments.length === 0) {
            console.warn("No original segments found in analyzer results");
            return null;
        }
        
        // Save the original audio descriptions to blob storage
        await uploadToBlob(JSON.stringify(segments, null, 2), title, title + ".json", null);
        
        return segments;
    } catch (error) {
        console.error("Error fetching or converting original analyzer results:", error);
        return null;
    }
};
...
/**
 * Extracts segments from analyzer results and converts them to original format (prioritizing summaryDescription)
 */
const extractOriginalSegmentsFromAnalyzerResults = (analyzerResults: ContentUnderstandingResults): Segment[] => {
    const segments: Segment[] = [];
    
    if (!analyzerResults.result?.contents) {
        console.warn("Original: No contents found in analyzer results");
        return segments;
    }
    
    console.log("Original: Processing analyzer results with", analyzerResults.result.contents.length, "content items");
    
    analyzerResults.result.contents.forEach((content, contentIndex) => {
        console.log(`Original: Processing content ${contentIndex}:`, content);
        let contentSegments: any[] = [];
        
        // First priority: Check for fields.Segments structure (original source)
        if (content.fields?.Segments) {
            // Check if it's the new simplified structure (array)
            if (Array.isArray(content.fields.Segments)) {
                console.log(`Original: Using contents[${contentIndex}].fields.Segments array data`);
                contentSegments = content.fields.Segments.map((segment: any) => ({
                    SegmentId: segment.SegmentId || segment.segmentId,
                    StartTimeMs: segment.StartTimeMs || segment.startTimeMs,
                    EndTimeMs: segment.EndTimeMs || segment.endTimeMs,
                    summaryDescription: segment.SummaryDescription || segment.summaryDescription || segment.description
                }));
            }
            // Check if it's the old nested structure with valueArray
            else if ('valueArray' in content.fields.Segments && content.fields.Segments.valueArray) {
                console.log(`Original: Using contents[${contentIndex}].fields.Segments.valueArray data (nested structure)`);
                contentSegments = content.fields.Segments.valueArray.map((item: any) => ({
                    SegmentId: item.valueObject.SegmentId.valueString,
                    StartTimeMs: item.valueObject.StartTimeMs?.valueInteger || parseInt(item.valueObject.StartTimeMs?.valueString || "0"),
                    EndTimeMs: item.valueObject.EndTimeMs?.valueInteger || parseInt(item.valueObject.EndTimeMs?.valueString || "0"),
                    summaryDescription: item.valueObject.SummaryDescription?.valueString || item.valueObject.description?.valueString
                }));
            }
        }
        // Fallback: Check for direct segments array 
        else if (content.segments && Array.isArray(content.segments)) {
            console.log(`Original: Found direct segments array in content ${contentIndex} with ${content.segments.length} segments`);
            contentSegments = content.segments.map((segment: any) => ({
                SegmentId: segment.segmentId || segment.SegmentId,
                StartTimeMs: segment.startTimeMs || segment.StartTimeMs,
                EndTimeMs: segment.endTimeMs || segment.EndTimeMs,
        ...
        console.log(`Original: Extracted ${contentSegments.length} segments from content ${contentIndex}`);
        
        // Convert to the required Segment format
        contentSegments.forEach((segment, segmentIndex) => {
            console.log(`Original: Processing segment ${segmentIndex} from content ${contentIndex}:`, segment);
            
            // Get the summaryDescription from the segment (prioritize original summary)
            let description = segment.summaryDescription || segment.SummaryDescription || segment.description || "";
            
            // If still empty, try to find any field that might contain the description
            if (!description && typeof segment === 'object') {
                for (const key of Object.keys(segment)) {
                    if (key.toLowerCase().includes('summary') || key.toLowerCase().includes('description')) {
                        description = segment[key];
                        console.log(`Original: Found description in key "${key}":`, description);
                        break;
                    }
                }
            }
            
            console.log(`Original: Final description for content[${contentIndex}].segments[${segmentIndex}]:`, description);
            ...
            segments.push({
                startTime: msToTime(startTimeMs),
                endTime: msToTime(endTimeMs),
                description: description || `Original Segment ${segments.length + 1} (content ${contentIndex}, segment ${segmentIndex})`
            });
        });
    });
    
    console.log(`Original: Total segments extracted: ${segments.length}`);
